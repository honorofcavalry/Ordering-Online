...
<script>
function updateorder(order_status) {
	alert("order_status");
	if(order_status="确认送达")
	{
		if(confirm("确定收到了吗？"))
		{
		$.post("order/songda.html",{order_status:order_status},function(date){
	...
	}else if(order_status="马上评价"){
		$.post("order/pingjia.html",{order_status:order_status},function(date){
...
<span>
    ...
    <tr>
     ...
     <tr>
      <td class="FontW">${c.order_id }</td>
      <td>${c.order_time }</td>
      <td>${sessionScope.user.user_name }</td>
      <td>￥${c.order_total }</td>
      <td><a href="order/select.html?order_status=${c.order_status}">${c.order_status}</a></td>
      <td><a href="order/delete.html?order_id=${c.order_id }">删除</a></td>
     </tr>     
       </c:forEach>
    </table>
<!--     <div class="TurnPage">
         ...
         <a href="#"><span class="Next">最后一页</span></a>
     </div> -->
  </span>
 </article>
</section>
